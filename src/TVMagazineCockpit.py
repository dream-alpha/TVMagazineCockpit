...
from .ChannelUtils import readChannelList, readChannelDict


class TVMagazineCockpit(Screen, More, Menu):
	...
	def showMenu(self):
		...
		if self.download.finished():
			self.openMenu(self.bouquet, self.channel_list, self.channel_dict)
	...
	def showMore(self):
		if self.download.finished():
			...
			if channel:
				...
				self.openMore(channel["service"], event)
